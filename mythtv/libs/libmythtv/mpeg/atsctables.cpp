    ...
    uint indent_level, uint chan) const
{
    ...
    str += QString("access_controlled=\"%1\"")
        .arg(QVariant(IsAccessControlled(chan)).toString());
    str += "\n" + indent_0;

    str += QString("hidden=\"%1\" ")
        .arg(QVariant(IsHidden(chan)).toString());
    str += QString("hide_guide=\"%1\"")
        .arg(QVariant(IsHiddenInGuide(chan)).toString());
    str += "\n" + indent_0;
    ...
    uint indent_level, uint chan) const
{
    ...
    return channel_info +
        ...
        .arg(QVariant(IsPathSelect(chan)).toString(),
             QVariant(IsOutOfBand(chan)).toString());
}
...
QString SystemTimeTable::toStringXML(uint indent_level) const
{
    ...
    return QString(
        ...
        .arg(indent_0,
             ...
             indent_1,
             QVariant(InDaylightSavingsTime()).toString(),
